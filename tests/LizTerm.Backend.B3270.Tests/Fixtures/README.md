...
Mint desktop runs in a language that writes decimals with a comma, and the bundled 4.5ga6 engine calls
`setlocale(LC_ALL, "")` and prints JSON doubles with `%g`, so its `run-result` reads `"time":0,039` and every `thumb`
line is malformed too. Those lines are not JSON and the parser drops them; the connection states and the screen
