...
- **The file system routes.** A `HostPathKind.Unix` path goes to `restfiles/fs/<path>` through `Route`, escaped
  ...
  UNIX path. `RenameAsync` refuses a UNIX path: the host has no rename. The cap (`uss-limits`) is Core's
  `HostFileLimits.MaxUnixFileBytes`, 1 MiB as measured on the tested build (the documented 64 KB is stale),
  checked before a request; a body past the host's own ceiling, about 2 MB, is a 400 "Failed to read request
  body" with nothing written.
