...
- **A build that is not a release says so, in About and on the splash** (#141). `AppVersion.Display` is the one
  spelling both windows show: a release is its bare version, anything else reads `Version 0.7.1-DEV (a1b2c3d)`.
  ...
  null outside a git checkout, where the line is just `0.7.1-DEV`. `AppVersion.Current` stays a bare `0.7.1` —
