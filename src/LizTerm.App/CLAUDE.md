...
- **The crosshair is a hairline on a cell boundary, not a shaded cell** (#180): the horizontal line on the cursor
  ...
  wants. `Rects` rounds every edge to a whole pixel, the run plan's underline rule: a cell is a fraction of one tall,
  and a hairline spread over two rows of pixels at partial coverage draws as a soft smear rather than a line.
