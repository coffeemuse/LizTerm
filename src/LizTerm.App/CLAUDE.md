...
- **Colours are decided in `Rendering/CellColors`, not in the control** (#123). `Monochrome` (a styled property the
  ...
  and no other background; the cell's own colour is never read. That is deliberate: b3270 sends no `fg` for a 3278
...
- **Four tabs** — Connection, Terminal, Organize, mvsMF — in Preferences' shape (a 120 px label column, labels at the top
...
- `TagBox` is an `AutoCompleteBox` inside a drawn `Border`, sharing a `WrapPanel` with the chips. `TagChip.ToString`
  ...
  never reaches the box, so the window posts the text back (the Wire Log correction's problem). And **a suggestion
  is chosen on the press**, in a tunnelled `PointerPressed` handler on the box that marks the press handled before the
  row sees it (the press routes out of the drop-down's popup through the box). Left to the row, in 12.1.2 the press
  selects it, writing its text into the box, and takes focus from the box, whose `OnLostFocus` closes the drop-down;
  on macOS the drop-down is a native popup, so the release that commits the choice never arrives and the text sat in
  the box with no chip (Robert's in-app pass, twice: a `DropDownClosed` handler reading `SelectedItem`, then a
  release handler, both passed headless tests, where the popup is an overlay and the release still lands).
  `A_pressed_suggestion_becomes_a_chip_without_a_release` presses without releasing for that reason. A
...
- `Render` emits one `<pre>` of `<span>` runs segmented by `Cell.SameStyleAs`, the renderer's own predicate. Both
  entry points take `monochrome`, which the view model passes from `Monochrome`, so a mono session's capture is the
  phosphor on black like its screen (`ScreenHtml.PhosphorHex`), reverse video swapped, whatever the cells carry.
...
- **A build that is not a release says so, in About and on the splash** (#141). `AppVersion.Display` is the one
  ...
  not, and it is the first control in the panel.
