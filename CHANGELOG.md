...
- **The crosshair is a thin line** along the edge of the cursor's cell, instead of a shaded row and column over the
  host's text ([#180](https://github.com/coffeemuse/LizTerm/issues/180), reported by
  [@mgrossmann](https://github.com/mgrossmann)).
- **Shift+End jumps the cursor to the end of a field.** The new **Field End** key sits where Vista TN3270 has it,
  and End itself still erases to the end of the field ([#177](https://github.com/coffeemuse/LizTerm/issues/177),
  reported by [@mgrossmann](https://github.com/mgrossmann)).
- **Logging out of macOS no longer stops to ask.** A logout, restart or shut down with sessions connected now
  disconnects them without the "still connected" question, as on Windows and Linux; Cmd+Q and closing a connected
  window still ask ([#169](https://github.com/coffeemuse/LizTerm/issues/169)).
- **A hand-edited `keymap.json` says what went wrong.** **Preferences > Keyboard** names each entry it skipped and
  why, and a file that is not valid JSON is reported at startup with the line to look at; **Reset keymap to
  defaults** moves it aside to `keymap.json.bad` ([#168](https://github.com/coffeemuse/LizTerm/issues/168)).
- **The mvsMF sign-in window reads more clearly**: it says why it is asking, an expired session is no longer shown
  in the same red as a refused password, and a blank box says which one is blank
...
- **Sessions no longer drop after thirty seconds on Linux desktops whose language writes decimals with a comma**,
  such as German, French or Croatian ([#170](https://github.com/coffeemuse/LizTerm/issues/170)).
- **About LizTerm's engine line can be copied**, like the version line above it, for bug reports.
